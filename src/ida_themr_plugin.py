...
if idaapi.is_idaq():
    # Import Qt classes through compatibility shim
    from qt_shim import (
        Qt,
        QEvent,
        QTimer,
    ...
    class StyleSheetWidget(QWidget):
        def __init__(self, parent=None):
            ...
            # Shortcuts
            QShortcut(QKeySequence(Qt.Key_F3), self).activated.connect(
                self.onNextSearchHit
            )
            ...
            ).activated.connect(self.onUndo)
            QShortcut(
                QKeySequence(Qt.CTRL + Qt.ALT + Qt.Key_Y), self
            ).activated.connect(self.onRedo)
    ...
    class ObjectInspector(QWidget):
        ...
        def _create_ui(self):
            ...
            self.chk_expand_on_updates.stateChanged.connect(
                self.toggle_expand_on_updates
            )
            ...
            self.tree_inspection.setSizePolicy(
                QSizePolicy.Expanding, QSizePolicy.Expanding
            )
            self.tree_inspection.setContextMenuPolicy(Qt.CustomContextMenu)
            self.tree_inspection.customContextMenuRequested.connect(
        ...
        def copy_context_category(self):
            ...
            if (
                not self.context_menu_data
                or self.context_menu_data["type"] != "category"
            ):
                return
            ...
            self.set_status_message(
                f"Category '{category_name}' copied to clipboard", 3000
            )
        ...
        def copy_context_property(self):
            ...
            if (
                not self.context_menu_data
                or self.context_menu_data["type"] != "property"
            ):
                return
        ...
        def widget_at_cursor(self):
            """Return the deepest child widget under the global cursor position."""
            pos = QCursor.pos()
        ...
        def eventFilter(self, obj, event):
            ...
            # Handle Enter events for widget inspection
                ...
                # find the real widget under the cursor (skips viewports, frames, etc.)
                if not self._update_suspended_state():
                    real = self.widget_at_cursor()
                    if real and real is not self and not self.isAncestorOf(real):
                        self._inspect_widget(real)
            return super().eventFilter(obj, event)

        def select_parent(self):
            # Jump "one level up" from whatever is currently under the cursor
            current = self.widget_at_cursor()
            if not current:
                return

            # If they really hit a viewport, widgetAt will skip it for us,
            # so current is your true child.  Now parent() it once:
        ...
        def qt_inheritance_chain(self, widget):
            chain = []
            mobj = widget.metaObject()
            while mobj is not None:
                chain.append(mobj.className())
        ...
        def _inspect_widget(self, widget):
            ...
            self._add_property(
                self.categories["Screen Information"], "DPI", f"{dpi:.1f}"
            )
            ...
            # C++ (Qt) base classes
            qt_chain = self.qt_inheritance_chain(widget)
            # drop the widget's own class, join the rest
            bases_cpp = qt_chain[1:] if len(qt_chain) > 1 else []
            self._add_property(
                self.categories["Basic Information"],
                "C++ Base Classes",
                ", ".join(bases_cpp) or "<none>",
            )

            py_mro = [cls.__name__ for cls in inspect.getmro(widget.__class__)]
            self._add_property(
                self.categories["Basic Information"],
                "Python MRO",
                " → ".join(py_mro),
    ...
    class QtStyleSelector(QWidget):
        ...
        def load_saved_settings(self):
            try:
                ...
                QMessageBox.warning(
                    self, "Warning", f"Failed to load settings: {str(e)}"
                )
    ...
    class IDAThemingToolkit(QMainWindow):
        ...
        def load_initial_settings(cls):
            try:
                ...
                # Set High DPI attributes using compatibility shim
                # The only minor consideration is that setHighDpiScaleFactorRoundingPolicy()
                # is ideally called before creating the QApplication, but since IDA already
                # has a QApplication running, calling it here is safe—it may have limited
                # effect in Qt6, but it won't cause errors.
                set_high_dpi_attributes()

                idaapi.msg(
                    f"{PLUGIN_NAME}: Using {QT_BINDING} (Qt {QT_VERSION}). Applied style: {style}, Dark mode: {dark_mode}{os.linesep}"
                )
            except Exception as e:
                if cls is IDAThemingToolkit:
                    idaapi.msg(f"Failed to load settings: {str(e)}\n")
                else:
...
class IDARootPlugin(idaapi.plugin_t):
    ...
    def init(self):
        # Only initialize if we're using the Qt version of IDA
...
if __name__ == "__main__" and idaapi.is_idaq():
    toolkit = IDAThemingToolkit()
    toolkit.show()
